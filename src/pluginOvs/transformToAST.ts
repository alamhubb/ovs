import {OvsParser, OvsSyntaxName} from "./OvsParser.ts";
import {tokenize} from "./ecma5/ecma5_lexer.ts";
import ChevrotainEcma5Cst from "./ChevrotainEcma5Cst.ts";
...
import ChevrotainEcma5Ast from "./ChevrotainEcma5Ast.ts";
import {
    ArgumentsExtendNode,
...
export function convertCstToChevrotainAst(cst: ChevrotainEcma5Cst): ChevrotainEcma5Ast {
    const result = {...cst, children: []};

    if (result.tokenTypeIdx) {
        result.tokenType = tokenIndexMap.get(result.tokenTypeIdx)
    }
    // {additionExpression:[]}
